  ...
  return (
    <section className="relative w-full bg-background/70 py-24 px-4 md:px-16">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          <span className="text-sm font-body text-muted-foreground">{t('projects.label')}</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sorted.map((repo, index) => (
            <RepositoryCard key={`${repo.owner}/${repo.name}`} repo={repo} index={index} />
          ))}
          <a
          ...
          >
            <div className="font-display italic text-4xl leading-tight">
              {t('projects.moreOnGithub.line1')}
              <br />
              {t('projects.moreOnGithub.line2')}
            </div>
